...
console.log("sign SHA-256 hash sample");
...
napiServer.onmessage = function (event) {
  ...
  var hash = sjcl.codec.hex.fromBits(sjcl.hash.sha256.hash("hello? hello?"))
  ...
  for (var i = 0; i < n; i++) {
    ...
    napiServer.send(JSON.stringify(req));
    console.log(i, "requisting sig for:", authenticated[i])
  }
...
napiServer.waitForOutcomes = function (event) {
  ...
  if (("sign" == outcome.op) && ("run" == outcome.subop)) {
    if(outcome.successful){
      ...
      console.log("  verified         :", outcome.verified)
      console.log("  sig              :", outcome.sig)
      console.log("  verificationKey  :", outcome.verificationKey)
      console.log("  verificationKeyId:", outcome.verificationKeyId)
    }
    else{
      console.log("FAILED sign:", outcome)
    }
  }
  else {
